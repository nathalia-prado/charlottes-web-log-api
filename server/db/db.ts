...
export function getAllPosts(): Promise<Post[]> {
  return db('posts').select(['id','title','date_created as dateCreated', 'text'])
}
...
export function getAllComments(id: number): Promise<Comment[]> {
  return db('comments').select(['id','comment','date_posted as datePosted', 'post_id as postId']).where({post_id: id})
}
